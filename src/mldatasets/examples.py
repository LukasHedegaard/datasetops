...
def domain_adaptation_office31(
    source_data_path:AnyPath, 
    target_data_path:AnyPath, 
    seed:int=0,
) -> Tuple[mlds.Dataset, mlds.Dataset, mlds.Dataset]:

    # load data
    source = load_folder_class_data(source_data_path).set_item_names('s_data', 's_label')
    target = load_folder_class_data(target_data_path).set_item_names('t_data', 't_label')

    num_source_per_class = 20 if 'amazon' in str(source_data_path) else 8
    num_target_per_class = 3

    source_train = source.shuffle(seed).filter(s_label=mlds.allow_unique(num_source_per_class))

    target_test, target_trainval  = target.split(fractions=[0.3, 0.7], seed=42) # hard-coded seed
    target_train, target_val = target_trainval.shuffle(seed).filter_split(t_label=mlds.allow_unique(num_target_per_class))
